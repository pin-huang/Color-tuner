...
class ViewController: UIViewController {
    ...
    // 設置元件 UIView (名為 clothesView)，並以 IBOutlet 連結到圖檔 clothes (等一下會拿來當作變色區塊的遮罩)
    @IBOutlet weak var clothesView: UIView!

    // 設置元件 UIView (名為 noseView)，並以 IBOutlet 連結到圖檔 nose (等一下會拿來當作變色區塊的遮罩)
    @IBOutlet weak var noseView: UIView!
        
    // 設置元件 UIImageView (名為 MickeyImageView)，並以 IBOutlet 連結到圖檔 Mickey (當作底圖)
    ...
    // 設置元件 UISegmented Control 並以 IBOutlet 連結至程式中的 IBAction function changingColorSlidder
    ...
    override func viewDidLoad() {
        ...
        let noseImageView = UIImageView(image: UIImage(named: "nose")) // 宣告一個名為 noseViewImageView 的常數，並存入名為 nose 的圖檔
        noseImageView.frame = clothesView.bounds // 將 noseImageView (UIImageView) 的框架尺寸調整成跟 clothesView (UIView 的 IBOutlet) 一樣
        noseImageView.contentMode = .scaleAspectFit // 設定 noseImageView (UIImageView) 的顯示模式為 Aspect fit
        noseView.mask = noseImageView // 使用 noseImageView (UIImageView) 當作 noseView (UIView 的 IBOutlet) 的遮罩
        noseView.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 1) // 預設啟動時鼻子顏色為黑色
    ...
    @IBAction func changingColorSlidder(_ sender: UISlider) {
        gradientView.removeFromSuperview() // 將漸層效果移除。為了在使用漸層後也可繼續使用其它的 slider 調色，必須加入這一行

        
        // 將各個 slider 的數值顯示在 IBOutlet redSliderValueDisplay (UILabel) 上
        ...
        // 設定以元件 Segmented control 來選擇變色區塊為鼻子或衣服，並連動到 IBOutlet segmentedControl
        let segmentSelect = segmentedControl.selectedSegmentIndex
        switch segmentSelect {

        case 0:
            // noseView 的顏色隨 slider 變化
            noseView.backgroundColor = UIColor(red: CGFloat(redSlider.value), green: CGFloat(greenSlider.value), blue: CGFloat(blueSlider.value), alpha: CGFloat(alphaSlider.value))

        case 1:
            // clothesView 的顏色隨 slider 變化
    ...
    @IBAction func gradientSlider(_ sender: UISlider) {
        ...
        gradientLayer.frame = noseView.bounds //設定漸層效果的圖層框架要與 noseView 一致
        ...
        case 0:
            ...
            noseView.addSubview(gradientView)
    ...
    @IBAction func randomize(_ sender: UIButton) {
        ...
        case 0:
            ...
            alphaSlider.value = 1 // 故意設成不透明，才不會讓底圖的顏色一直跑出來
